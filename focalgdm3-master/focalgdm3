...
if [ "$codename" == "focal" ] || [ "$codename" == "groovy" ]
then
source="/usr/share/gnome-shell/theme/Yaru/gnome-shell-theme.gresource"
else
echo "
----------------------------------------
Sorry, Script is only for Ubuntu 20.xx
Exiting...
----------------------------------------"
exit 1
fi
...
if [ "$UID" != "0" ]
then
echo -e "This Script must be run with root privileges; example: \"sudo ./focalgdm3\""
exit 1
fi
...
-----------------------------------------------------------------------------------------------------"
echo "Would you like to install libglib2.0-dev? y/N"
read ans
  if [ "$ans" == "y" ] || [ "$ans" == "Y" ]; then
...
*) #########################################################################################
############################################################################################
echo -e "\nUse the parameter '--set' or '--reset'; example: './focalgdm3 --set' or './focalgdm3 --reset'\n"
exit 1
esac
